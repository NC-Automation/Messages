...
import com.ncautomation.messages.extensions.format
import com.ncautomation.messages.helpers.parseNameFromVCard
    ...
    companion object {
        ...
        private fun VCardProperty.getPropertyTypeString(context: Context): String {
            return when (parameters.type) {
                CELL -> context.getString(com.ncautomation.commons.R.string.mobile)
                HOME -> context.getString(com.ncautomation.commons.R.string.home)
                WORK -> context.getString(com.ncautomation.commons.R.string.work)
                else -> ""
            }
        }
            ...
            return property.run {
                when (this) {
                    ...
                    is Organization -> VCardPropertyWrapper(
                        ...
                        type = context.getString(com.ncautomation.commons.R.string.work),
                        property = property
                    )
                    is Birthday -> VCardPropertyWrapper(
                        value = date.format(context.config.dateFormat),
                        type = context.getString(com.ncautomation.commons.R.string.birthday),
                        property = property
                    )
                    is Anniversary -> VCardPropertyWrapper(
                        value = date.format(context.config.dateFormat),
                        type = context.getString(com.ncautomation.commons.R.string.anniversary),
                        property = property
                    )
                    is Note -> VCardPropertyWrapper(value, context.getString(com.ncautomation.commons.R.string.notes), property)
